...
#4 pred - residual ratings calculated from above and then from matrix factorizaton
...
f_rmses[2]<-RMSE(predicted_ratings_0.015,edx_test$rating)
# runs the Simon Funk's stocastic gradient descent algorithm to factorize matrix of residuals
fsvd<-funkSVD(y, k=3, gamma=gammas[1], lambda=0.001, verbose=TRUE)
# y_hat is prediction matrix from SVD 
y_hat_0.01<-tcrossprod(fsvd$U,fsvd$V)
# Assigns row and column names to prediction matrix so that predicted rating can be pulled from the matrix for any user i and movie j
rownames(y_hat_0.01)<-rownames(y)
colnames(y_hat_0.01)<-colnames(y)
pred<-rep(0,length(edx_test$userId))
# fill vector pred from the prediction matrix for respective userId and movieId
for(i in 1:length(edx_test$userId)){
  pred[i]<-y_hat_0.01[as.character(edx_test$userId[i]),as.character(edx_test$movieId[i])]}
# calculated predicted ratings form edx_test for that particular gamma
predicted_ratings_0.01<-predicted_ratings_test+pred
# calculates rmse for that particular gamma
f_rmses[1]<- RMSE(predicted_ratings,edx_test$rating)
###########################################################################
# runs the Simon Funk's stocastic gradient descent algorithm to factorize matrix of residuals
fsvd<-funkSVD(y, k=3, gamma=gammas[3], lambda=0.001, verbose=TRUE)
y_hat_0.02<-tcrossprod(fsvd$U,fsvd$V)
rownames(y_hat_0.02)<-rownames(y)
colnames(y_hat_0.02)<-colnames(y)
pred<-rep(0,length(edx_test$userId))
for(i in 1:length(edx_test$userId)){
  pred[i]<-y_hat_0.02[as.character(edx_test$userId[i]),as.character(edx_test$movieId[i])]}
predicted_ratings_0.02<-predicted_ratings_test+pred
f_rmses[3]<- RMSE(predicted_ratings_0.02,edx_test$rating)
...
predicted_ratings_valid<-
  ...
  mutate(pred=mu+b_i+b_u)%>%
  pull(pred)
# This fives rmse before applying SVD to residual
